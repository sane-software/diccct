...
// SaneDiccct is a menu-bar-only macOS app with zero third-party dependencies by
// design: only Apple's own Foundation / AppKit / SwiftUI are used.
//
// Layout:
//   - SaneDiccctCore  : pure, UI-agnostic logic (dict.cc parsing, search index,
//                   data-directory scanning). Foundation-only, so it is fully
//                   unit-testable without spinning up AppKit.
//   - SaneDiccct      : the executable app shell (AppKit status item + non-activating
...
// wraps the built binary into a SaneDiccct.app bundle for normal use.
let package = Package(
    name: "SaneDiccct",
    platforms: [
    ...
    targets: [
        .target(
            name: "SaneDiccctCore",
            path: "Sources/SaneDiccctCore"
        ),
        .executableTarget(
            name: "SaneDiccct",
            dependencies: ["SaneDiccctCore"],
            path: "Sources/SaneDiccct",
            swiftSettings: [
        ...
        .testTarget(
            name: "SaneDiccctCoreTests",
            dependencies: ["SaneDiccctCore"],
            path: "Tests/SaneDiccctCoreTests"
        ),
    ]
)
